...
flowchart TB
    subgraph LAN["LAN — 10.10.x.0/24"]
        ...
        TNAS[TrueNAS<br/>movies NFS]
        APPS -->|jellyfin static PV ro| TNAS
        DOCK1 -->|fstab mount rw| TNAS
...
| **truenas** | `__LAN-IP__` | TrueNAS SCALE 25.10.4 — movies library on 2×8TB ZFS mirror, NFS | Bare metal. Serves Jellyfin (read-only static PV) and qbittorrent downloads (docker-1 fstab mount). Weekly config export → Synology → Wasabi. UI: https://nas2.__BASE-DOMAIN__ (Traefik ext-truenas bundle). |
